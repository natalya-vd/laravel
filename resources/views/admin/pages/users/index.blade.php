        ...
        <table>
            ...
            <tr>
                ...
                <td class="table__cell">
                    <input type="checkbox" {{$item->is_admin == 1 ? 'checked' : ''}} disabled>
                </td>
                <td class="table__cell">
                    {{!is_null($item->created_at) ? $item->created_at->format('d-m-Y H:i') : ''}}
                </td>
                <td class="table__cell">
                    <form action="{{route('admin.users.destroy', $item)}}" method="post" enctype="multipart/form-data">
                        @csrf
                        <a href="{{route('admin.users.edit', $item)}}" class="btn btn-primary">
